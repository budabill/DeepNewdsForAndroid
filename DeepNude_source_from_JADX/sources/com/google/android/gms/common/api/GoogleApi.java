...
public class GoogleApi<O extends ApiOptions> {
    ...
    private final zzs zzahx() {
        ...
        Collection zzacf;
        zzs zzs = new zzs();
        ApiOptions apiOptions = this.zzfsm;
        if (apiOptions instanceof HasGoogleSignInAccountOptions) {
            googleSignInAccount = ((HasGoogleSignInAccountOptions) apiOptions).getGoogleSignInAccount();
            if (googleSignInAccount != null) {
                account = googleSignInAccount.getAccount();
                zzs = zzs.zze(account);
                apiOptions = this.zzfsm;
                if (apiOptions instanceof HasGoogleSignInAccountOptions) {
                    googleSignInAccount = ((HasGoogleSignInAccountOptions) apiOptions).getGoogleSignInAccount();
                    if (googleSignInAccount != null) {
        ...
        apiOptions = this.zzfsm;
        account = apiOptions instanceof HasAccountOptions ? ((HasAccountOptions) apiOptions).getAccount() : null;
        zzs = zzs.zze(account);
        apiOptions = this.zzfsm;
        if (apiOptions instanceof HasGoogleSignInAccountOptions) {
            googleSignInAccount = ((HasGoogleSignInAccountOptions) apiOptions).getGoogleSignInAccount();
    ...
    @Hide
    public final <A extends zzb, T extends zzcq<A, ?>, U extends zzdo<A, ?>> Task<Void> zza(T t, U u) {
        zzbq.checkNotNull(t);
        zzbq.checkNotNull(u);
        String str = "Listener has already been released.";
        zzbq.checkNotNull(t.zzakx(), str);
        zzbq.checkNotNull(u.zzakx(), str);

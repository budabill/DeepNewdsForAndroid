...
public abstract class zzg<T> extends AbstractDataBuffer<T> {
    ...
    public final T get(int i) {
        ...
        int zzcb = zzcb(i);
        if (i >= 0) {
            if (i != this.zzgcz.size()) {
                intValue = (i == this.zzgcz.size() - 1 ? this.zzfxb.zzgcq : ((Integer) this.zzgcz.get(i + 1)).intValue()) - ((Integer) this.zzgcz.get(i)).intValue();
                if (intValue == 1) {
                    this.zzfxb.zzby(zzcb(i));
                }
                return zzl(zzcb, intValue);
            }
        }
        intValue = 0;
        return zzl(zzcb, intValue);

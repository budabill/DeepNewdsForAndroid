...
public class GoogleApiAvailability extends zzf {
    ...
    class zza extends Handler {
        ...
        public final void handleMessage(Message message) {
            ...
            i = this.zzfqy.isGooglePlayServicesAvailable(this.mApplicationContext);
    ...
    private final void zza(Context context, int i, String str, PendingIntent pendingIntent) {
        if (i == 18) {
            ...
            if (zzj.zzcv(context)) {
                ...
                Notification.Builder addAction = new Notification.Builder(context).setSmallIcon(context.getApplicationInfo().icon).setPriority(2).setAutoCancel(true).setContentTitle(zzg).setStyle(new BigTextStyle().bigText(zzi)).addAction(C0562R.drawable.common_full_open_on_phone, resources.getString(C0562R.string.common_open_on_phone), pendingIntent);
                if (zzs.isAtLeastO() && zzs.isAtLeastO()) {
                    addAction.setChannelId(zza(context, notificationManager));
                }
                build = addAction.build();
            } else {
                NotificationCompat.Builder style = new NotificationCompat.Builder(context).setSmallIcon(17301642).setTicker(resources.getString(C0562R.string.common_google_play_services_notification_ticker)).setWhen(System.currentTimeMillis()).setAutoCancel(true).setContentIntent(pendingIntent).setContentTitle(zzg).setContentText(zzi).setLocalOnly(true).setStyle(new NotificationCompat.BigTextStyle().bigText(zzi));
                if (zzs.isAtLeastO() && zzs.isAtLeastO()) {
    ...
    public Task<Void> checkApiAvailability(GoogleApi<?> googleApi, GoogleApi<?>... googleApiArr) {
        ...
        zzbq.checkNotNull(googleApi, str);
        for (Object checkNotNull : googleApiArr) {
            zzbq.checkNotNull(checkNotNull, str);
        }
